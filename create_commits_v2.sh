...
create_commit() {
    ...
    for file in $files; do
        if [ -e "$file" ]; then
            # Check if file is not tracked, or force add it
            if ! git ls-files --stage "$file" | grep -q .; then
                git add "$file" 2>/dev/null || true
            else
                # File is tracked, add it anyway to show it in this commit
                git add "$file" 2>/dev/null || true
            fi
        fi
    done
...
# Commit 1: Initial project setup (Amir)
create_commit "chappalwalaamir@gmail.com" "Amir Chappalwala" "$DATE1" \
...
# Commit 7: Receipt upload endpoint (Ankur) - modify controller
